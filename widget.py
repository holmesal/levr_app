...
class LoggedInFavHandler(webapp2.RequestHandler):
	def get(self):
		...
		fav.dealID	= dealID
		fav.put()
		self.redirect('/widget/show?id='+businessID)
	...
	def post(self):
		try:
			...
			if response['success'] == True:
				...
				businessID 	= enc.encrypt_key(businessID)
				self.redirect('/widget/show?id=' + businessID + '&success=1')
			else:
				dealID = enc.encrypt_key(dealID)
				#show login page again, with login error
				self.redirect('/widget/show?id='+dealID+'&error=login&email_or_owner='+email_or_owner+'&action=login')
		except:
			levr.log_error()
class SignupFavHandler(webapp2.RequestHandler):
	def post(self):
		try:
			...
			if len(pw) < 5:
				dealID = enc.encrypt_key(dealID)
				self.redirect('/widget/show?deal='+dealID+'&id='+businessID+ '&error=password'+'&email='+email+'&username='+alias+'&action=login')
			else:
				session = get_current_session()
				...
				if response['success']:
					...
					session['loggedIn'] = True
					dealID = enc.encrypt_key(dealID)
					self.redirect('/widget/show?deal='+dealID+'&id='+businessID+ '&action=show')
				else:
					dealID = enc.encrypt_key(dealID)
